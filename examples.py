...
def SynthesisFromDOGActivity(pic):
    ...
    for i in range(pic.numScaleLevels):
        eidolonDataPlane += next(fiducialDOGScaleSpace) # this is a generator!
...
def SynthesisFromLaplacian(pic, INTEGRATION_FUDGE_FACTOR):
    ...
    eidolonDataPlane = np.zeros(pic.fatFiducialDataPlane.shape)
    #// here the eidolon is constructed
    eidolonDataPlane = next(rockBottomPlaneGenerator) + StackIntegrate(fiducialLaplacianScaleSpace, numScaleLevels, scaleLevels, picSize) * INTEGRATION_FUDGE_FACTOR
...
def SynthesisFromSimpleCellActivity(pic, INTEGRATION_FUDGE_FACTOR):
    ...
    eidolonDataPlane = next(rockBottomPlaneGenerator) + eidolonDataPlane * INTEGRATION_FUDGE_FACTOR * THREE_FOURTH
...
def OrientedEdgeDisarray(pic, reach, INTEGRATION_FUDGE_FACTOR):
    ...
    eidolonDataPlane = next(rockBottomPlaneGenerator) + (planeP + planeQ + planeR) * THREE_FOURTH * INTEGRATION_FUDGE_FACTOR
...
def DisplayScalespaceLevel(pic, theLevel):   
    ...
    i = 0
    while i <= theLevel:
        eidolonDataPlane = next(fiducialScaleSpace)
        i += 1
...
def DisplayFractalNoise(pic, theLevel):         
    ...
    i = 0
    while i <= theLevel:
        eidolonDataPlane = next(coherentRandomGaussianDataStack)
        i += 1
...
def DisplayGradientDirection(pic, theLevel):  
    ...
    i = 0
    while i <= theLevel:
        gx = next(fiducialFirstOrderXScaleSpace)
        gy = next(fiducialFirstOrderYScaleSpace)
        i += 1
...
def LineFinderField(pic, theLevel):
    ...
    i = 0
    while i <= theLevel:
        fiducialSecondOrderPScaleSpace, fiducialSecondOrderQScaleSpace, fiducialSecondOrderRScaleSpace = next(fiducialSecondOrder)
        i += 1    
